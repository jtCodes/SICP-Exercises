        ...
        (define (execute)
          (let ((insts (get-contents pc)))
            (cond ((null? insts) 'done)
                  ((exists? (car insts) breakpoints) 'break)
                  (else
        ...
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ((eq? message 'set-breakpoint)
                 (lambda (label offset)
                   (set! breakpoints
                         ;;loop-insts returns the instruction right before breaking
                         (cons (loop-insts (cdr (assoc label labels)) offset)
                               breakpoints))
                   'done))
                ((eq? message 'proceed)
                 (lambda ()
                ...
                ((eq? message 'cancel-breakpoint)
                 (lambda (label offset)
                   (set! breakpoints
                         (delete (loop-insts (cdr (assoc label labels)) offset)
                                 breakpoints))
                   'done))
...
(define (extract-labels text receive)
  (define inst-count 0)
  (define (extract-labels-iter text receive)
    (if (null? text)
         ...
         (lambda (insts labels)             ;; This is where labels and insts are
           (let ((next-inst (car text)))    ;;   accumulated (as on a stack):
             (cond ((symbol? next-inst)
                    (cond ((label-defined? labels next-inst)
                           (error next-inst " cannot be defined twice."))
                          (else
                           (receive insts                 ;; either label
                                    (cons (make-label-entry next-inst
                                                            insts)
                                          labels)))))
                   (else
                    (set! inst-count (+ inst-count 1))
                    ;;; Updated to include inst-number for breakpoint
                    (receive (cons (make-instruction next-inst inst-count) ;; or
                                   insts)                       ;; (it's an inst)
                             labels)
                    )))))))
  (extract-labels-iter text receive))
...
;;; set-instruction-execution-proc!
;;; *
;;; Updated to include inst-count for breakpoint
;;; inst-count act as id to differentiate between two same insts at
;;; different location 

(define (make-instruction text inst-count)
  (list text inst-count))
...
(define (loop-insts insts offset)
  (define (iter insts offset)
    (let ((inst-pair (cons (caar insts) (cadar insts))))
      (cond ((equal? 1 offset)
             (car insts))
            (else
             (iter (cdr insts) (- offset 1))))))
  (iter insts offset))

(define (loop-break breakpoint breakpoints)
  (cond ((null? breakpoints) #f)
        ((equal? breakpoint (car breakpoints)) breakpoint)
        (else
         (loop-break breakpoint (cdr breakpoints)))))
...
(define (exists? item list)
  (cond ((null? list) #f)
        ((equal? item (car list)) #t)
        (else (exists? item (cdr list)))))
