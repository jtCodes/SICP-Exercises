...
(define (make-operation-exp exp machine labels operations)
  (cond ((equal? (permitted-operand? (operation-exp-operands exp)) #f)
         (error "op can only operate on reg or const"))
        (else
...
;;;new
;;; (permitted-operand? '((reg b) (const 0)))
(define (permitted-operand? operands)
    (cond ((null? operands) #t)
          ((or (equal? (operand-type (car operands)) 'reg)
               (equal? (operand-type (car operands)) 'const))
           (permitted-operand? (cdr operands)))
          (else #f)))
